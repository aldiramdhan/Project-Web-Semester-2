...
<body>
    <div class="main d-flex justify-content-between flex-column">
        <nav class="navbar navbar-dark navbar-expand-lg bg-light shadow-sm sticky-top">
            <div class="container-fluid">
                <a class="navbar-brand text-black" href="#">
                    <img src="{{ asset('images/logo.svg') }}" alt="logo" height="80%" width="80%" >
                    </a>
        ...
        <div class="body-content h-100">
            <div class="row g-0 h-100">
                <div class="sidebar  col-lg-2 collapse d-lg-block bg-light" id="button-primary p-2">
                    @if(Auth::user())
                    @if(Auth::user()->role_id == 1)
                    <a href="/dashboard" class="{{ (request()->is('dashboard') ? 'active' : '') }}" class="far fa-user"> 
                        <i class="bi bi-house-door"></i>
                        Dashboard</a>
                    <a href="{{ route('books.index') }}"
                        class="{{ (request()->is('books*') ? 'active' : ''); }}">
                    ...
                    <a href="{{ route('rent_logs.index') }}"
                        class="{{ (request()->is('rent-logs*') ? 'active' : '') }}">
                    ...
                    <a href="{{ route('book-rent.index') }}"
                        class="{{ (request()->is('book-rent*') ? 'active' : '') }}">
